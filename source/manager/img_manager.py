...
class ImgIcon(AssetBase):
    def __init__(self,
        ...
        if self.offset != 0:
            self.cap_posi = list(
                np.array(self.cap_posi) + np.array([-self.offset, -self.offset, self.offset, self.offset]))

        self.cap_center_position_xy = [(self.cap_posi[0] + self.cap_posi[2]) / 2,
                                       (self.cap_posi[1] + self.cap_posi[3]) / 2]
